...
function mod:Deaths(unit)
	...
	elseif unit == L["Fathom-Guard Tidalvess"] then
		self:Bar("Next Spitfire", timer.totem, icon.totem) --Inital spitfire once shaman dies
	end
end
...
function mod:Totem(_, spellID, source)
	...
	if source == boss then
		...
		self:Bar("Next Spitfire", timer.totem, icon.totem)
	else
