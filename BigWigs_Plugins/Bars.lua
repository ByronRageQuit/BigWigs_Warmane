...
function plugin:OnEnable()
	...
	self:RegisterEvent("BigWigs_PauseCounterBar")
...
-----Counter Bar Testing------
function plugin:BigWigs_StartCounterBar(module, text, max, icon, bar, otherc, ...)
	if not text then return end
	local id = "BigWigsBar "..text
	self:BigWigs_StartBar(module, text, max, icon, bar, otherc, ...)
	self:SetCandyBarTimeFormat(id, function(t) return math.ceil(t) end)
	self:PauseCandyBar(id)
end

function plugin:BigWigs_StopCounterBar(module, text)
	local id = "BigWigsBar "..text
	if not text then return end
	self:BigWigs_StopBar(module, text)
end

function plugin:BigWigs_SetCounterBar(module, text, value)
	if (not text) or (value == nil) or (value < 0) then return end --Sanity check
	local id = "BigWigsBar "..text --Set bar id
	self:StartCandyBar(id)
	self:SetCandyBarTimeLeft(id, value)
	self:ScheduleEvent("JankySleep", "BigWigs_PauseCounterBar", 0.1, self, text)
end

function plugin:BigWigs_PauseCounterBar(module, text)
	if (not text) then return end
	local id = "BigWigsBar "..text --Set bar id
	self:PauseCandyBar(id)
end
