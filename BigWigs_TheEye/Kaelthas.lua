...
L:RegisterTranslations("enUS", function() return {
	...
	weapons_inc_trigger = "As you see, I have many weapons in my arsenal....", --Phase 2 trigger, added period to match warmane
...
function mod:CHAT_MSG_MONSTER_YELL(msg)
	...
	elseif msg == L["weapons_inc_trigger"] then --phase 2
		phase = 2
		self:Message(L["weapons_inc_message"], "Positive")
		self:Bar(L["revive_bar"], 101, "Spell_Holy_ReviveChampion") -- Weapons spawn 5 seconds after death of last advisor 102????
		--self:DelayedMessage(102, L["revive_warning"], "Attention") -- and it is 2 minutes from weapon spawn till advisor revive
	elseif msg == L["phase3_trigger"] then
		phase = 3
		self:Message(L["phase3_message"], "Positive")
		self:Bar(L["phase4_bar"], 183, "Spell_ChargePositive")
	elseif msg == L["phase4_trigger"] then
		phase = 4
		self:Message(L["phase4_message"], "Positive")
		if db.rebirth then
			self:Bar(L["rebirth_bar"], 30, "Spell_Fire_Burnout")
	...
	elseif db.rebirth and (msg == L["rebirth_trigger1"] or msg == L["rebirth_trigger2"]) then
		...
		self:Bar(L["rebirth_bar"], 50, "Spell_Fire_Burnout") 		--Seems like first is 30sec, rest are 50
