...
L:RegisterTranslations("enUS", function() return {
	...
	decay_bar = "~Possible Death & Decay~",

	icebolt = "~Possible Icebolt~",
...
function mod:Icebolt(player, spellID)
	if db.icebolt then
		...
		self:Bar(L["Icebolt"],7,"spell_frost_frostbolt02") --ideally should start timer on spellcast, not spell hit on player
	end
end

function mod:IceboltTimer(_, spellID)
	if db.icebolt then
		self:Bar(L["Icebolt"],7,"spell_frost_frostbolt02")
	end
end
...
function mod:DeathAndDecay(player, spellID)
	if db.decay then
		...
		self:Bar(L["decay_bar"],60,"ability_creature_cursed_02")
		self:Bar(L["Icebolt"],15,"spell_frost_frostbolt02") --D&D stops icebolts, but casts right after it finishes
	end
end
...
function mod:BigWigs_RecvSync(sync, rest, nick)
	if self:ValidateEngageSync(sync, rest) and not started then
		...
		if db.icebolt then
			self:Bar(L["Icebolt"],7,"spell_frost_frostbolt02") --8 is the average
		end
		if db.decay then
			self:Bar(L["decay_bar"],45,"ability_creature_cursed_02") --seems like the first D&D is faster
		end
	end
end
