...
function mod:CHAT_MSG_MONSTER_YELL(msg)
	...
	elseif msg == L["image_trigger"] then
		...
		self:TriggerEvent("BigWigs_StopBar", self, L["demon_nextbar"])
		--add whirlwind bar after split
		if db.whirlwind and demon_state == true then --If the split happens during demon phase then start new WW
			self:WhirlwindBar()
		end
		if db.image then
...
function mod:DemonSoon()
	demon_state = false --After 60 sec demon phase ends, and this function is run. So set state
