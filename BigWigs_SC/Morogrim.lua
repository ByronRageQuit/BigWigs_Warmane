...
L:RegisterTranslations("enUS", function() return {
	...
	murloc_yells1 = "By the tides!",
	murloc_yells2 = "Destroy them, my subjects!",
...
function mod:Tidal()
	if db.tidal then
		self:IfMessage(L["tidal_message"], "Urgent", 37730, "Personal")
	end
end
...
function mod:Grave(player)
	self:Bar(L["grave_nextbar"], 30, 37850)
...
function mod:CHAT_MSG_MONSTER_YELL(msg)
	...
	if msg == L["engage_trigger"] then
		...
		if db.murloc then  
			self:CancelScheduledEvent("murloc1")
			--self:IfMessage(L["murloc_message"], "Positive", 42365)
			self:Bar(L["murloc_bar"], 42, 42365)
			--self:ScheduleEvent("murloc1", "BigWigs_Message", 42, L["murloc_soon_message"], "Attention")
		end
		if db.grave then
			self:Bar(L["grave_nextbar"], 20, "Spell_Frost_ArcticWinds")
		end
	--Morogrim Spawns Murlocs
	elseif msg == L["murloc_yells1"] or msg == L["murloc_yells2"] then
		self:Murlocs()
	end
end

--Alert who is in  the watery graves
...
function mod:UNIT_HEALTH(msg)
	...
	if UnitName(msg) == boss then
		...
		if health > 26 and health <= 30 and not grobulealert then
			self:Message(L["globules_warning"], "Positive", "Alert")
			grobulealert = true
