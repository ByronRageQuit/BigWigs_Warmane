...
function mod:Swarm(_, spellID)
	if db.swarm then
		...
		self:Bar(L["swarm_bar"], 12, spellID)
	end
end
...
function mod:Inferno()
	if db.inferno then
		...
		self:Bar(L["inferno_bar"], 48, 31299)
...
function mod:BigWigs_RecvSync(sync, rest, nick)
	if self:ValidateEngageSync(sync, rest) and not started then
		...
		--Start inital timers
		if db.inferno then
			self:Bar(L["inferno_bar"], 50, 31299) --data has first at 50 rest at ~48
		end
		if db.swarm then
			self:Bar(L["swarm_bar"], 12, spellID) --avg is actually 14 but min is 12
		end
	end
end
